...
if __name__ == "__main__":
    try:
        ...
        client = AzureOpenAI(
            ...
            azure_endpoint="https://ptutt-mp6ndin8-canadaeast.cognitiveservices.azure.com/",
        )
            ...
            for i, line in enumerate(file):
                if i in inserts:
                    ...
                    if is_silent_wav(f"./results/output-{i}.wav"):
                        result = ""
                        print("No speech detected — skipping ASR")
                    else:
                    ...
                    synthesis(
                        tts_device, tts_model, vocoder, denoiser,
                        feedback_to_child, spk, LANGUAGE, fb_id
                    )
                    ...
                    messages.append({
                        "role": "assistant",
                        "content": f"You added to the story: {answer_to_child}"
                    })
                    
                    answer_id = f"answer-{i}"
                    synthesis(
                        tts_device, tts_model, vocoder, denoiser,
                        answer_to_child, spk, LANGUAGE, answer_id
                    )

                    answer_wav = f"{WAV_PATH}/to_play-{answer_id}.wav"
                    wait_done(answer_wav)
                    
                    with open("./results/transcription.txt", "a", encoding="utf-8") as transcripts:
                        transcripts.write(f"\n--- line {i} ---\n")
                        transcripts.write(f"Question: {inserts[i]}\n")
                        transcripts.write(f"Child response: {result}\n")
                        transcripts.write(f"LLM feedback: {feedback_to_child}\n")
                        transcripts.write(f"LLM story sentence: {answer_to_child}\n")
